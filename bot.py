...
class ServerStatusMonitor(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.server_status = None  # None signalisiert, dass wir den aktuellen Status nicht kennen
        self.server_monitor.start()

    @tasks.loop(seconds=30)  # Stellen Sie sicher, dass die Loop-Methode korrekt eingerückt ist
    async def server_monitor(self):
        host = "45.93.251.18"
        channel = self.bot.get_channel(1200120616230076496)
        
        # Hier fügen wir die Logik ein, um den Server zu pingen und den Status zu überprüfen
        current_status = await ping_server(host)
    ...
    @server_monitor.before_loop  # Der Dekorator muss direkt vor der Methode stehen
    async def before_server_monitor(self): # Stellen Sie sicher, dass auch diese Methode korrekt eingerückt ist
...
async def restart(ctx):
    ...
    if error:
        ...
        for i in range(10):
            if await ping_server(host):
                await ctx.send(f"✅ Dererver `{host}` ist wieder erreichbar.")
                return
            else:
                await asyncio.sleep(30)
